...
impl ConsoleParam {
    ...
    pub fn to_service(&self) -> Service {
        ...
        self.configure_unit_section(&mut svc);
        self.configure_service_section(&mut svc);
    ...
    fn configure_unit_section(&self, svc: &mut Service) {
        ...
        if self.is_serial() {
            let dev = format!("dev-{}.device", self.tty);
            svc.unit.after.push(dev.clone());
            svc.unit.requires.push(dev);
        }
    }

    fn configure_service_section(&self, svc: &mut Service) {
    ...
    fn agetty_command(&self) -> String {
        if !self.is_serial() {
            return format!("/sbin/agetty -o '-p -- \\\\u' --noclear {} $TERM", self.tty);
        }
        match self.baud {
            Some(baud) => format!(
